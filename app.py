...
# --- FUNÇÕES DE BANCO DE DADOS (SUPABASE API) ---
...
# Se estiver logado, exibe todo o sistema:
        ...
        if etapa_evento == "1. Planejamento (Criar & Orçar)":
            ...
            if not evs_plan.empty:
                ...
                with st.form("f_add_orc_ev"):
                    ...
                    if st.form_submit_button("Adicionar Linha ao Orçamento"):
            ...
            if not evs_encerrados.empty:
                ...
                st.markdown("---")
                if st.button("⚠️ Reabrir Evento (Devolver para Fase 'Live')", help="Caso tenha fechado sem querer e precise editar despesas ou inscrições."):
    ...
    # ==========================================
        ...
        with st.form("f_add_orc"):
            ...
            tipo_orc = c1.selectbox("Tipo de Orçamento", ["Despesa", "Receita"])
            cat_sel = c2.selectbox("Categoria Correspondente", categorias if tipo_orc == "Despesa" else ["Academia", "Personal", "Calistenia", "Geral"])
            mes_sel = c3.selectbox("Mês Específico", [f"{ano_atual}-{m:02d}" for m in range(1,13)])
            val_orc = c4.number_input("Valor Projetado (R$)", min_value=0.0, value=None)
            
            if st.form_submit_button("Salvar Orçamento", type="primary"):
                if val_orc is not None:
                    df_check = buscar_dados('orcamentos', eq={'ano': ano_atual, 'mes': mes_sel, 'categoria': cat_sel, 'tipo': tipo_orc})
                    if not df_check.empty:
                        atualizar_dados('orcamentos', {'valor': val_orc}, 'id', int(df_check.iloc[0]['id']))
                    else:
                        inserir_dados('orcamentos', {'ano': ano_atual, 'mes': mes_sel, 'categoria': cat_sel, 'valor': val_orc, 'tipo': tipo_orc})
                    st.session_state.sucesso_msg = "Orçamento atualizado e salvo com sucesso!"
                    st.rerun()
